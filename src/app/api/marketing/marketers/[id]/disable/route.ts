...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!isManager(session.role)) {
    return NextResponse.json({ success: false, error: 'Only managers or admins can disable accounts' }, { status: 403 });
  }
  if (params.id === session.marketerId) {
    return NextResponse.json({ success: false, error: 'You cannot disable your own account' }, { status: 403 });
  }
  ...
  if (!reason || reason.length < 10) {
    return NextResponse.json({ success: false, error: 'Reason required (min 10 characters)' }, { status: 400 });
  }

  const client = getRouteClient(request.headers.get('authorization'));
  const { error } = await client.rpc('disable_marketer', {
