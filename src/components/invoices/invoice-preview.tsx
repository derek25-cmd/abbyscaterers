            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1">
                <div className="text-base">
                  {formData.receiverName && <p className="mb-1"><strong>To:</strong> {formData.receiverName}</p>}
            ...
            <table className="w-full border-collapse border border-gray-800 mb-3 text-sm">
              ...
              <tbody>
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="border border-gray-800 p-1.5">
                      {item.particularType === 'event'
                        ? `${item.eventType === 'Custom' ? (item.customEventType || '{EventType}') : (item.eventType || '{EventType}')}${item.date ? ` on ${formatDate(item.date)}` : ''}`
                        : `${item.mealType || '{MealType}'}${item.date ? ` on ${formatDate(item.date)}` : ''}`
                      }
                    </td>
                ...
                <tr>
                  ...
                  <td className="border border-gray-800 p-1.5 text-right">{formData.serviceCharge > 0 ? formData.serviceCharge.toLocaleString() : '0.00'}</td>
                </tr>
                <tr>
                  <td colSpan={3} />
                  <td className="border border-gray-800 p-1.5 text-right">Add Transportation Costs (TSHS)</td>
                  <td className="border border-gray-800 p-1.5 text-right">{formData.transportCosts > 0 ? formData.transportCosts.toLocaleString() : '0.00'}</td>
                </tr>
                <tr>
                  <td colSpan={3} />
