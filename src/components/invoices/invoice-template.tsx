...
export function InvoiceTemplate({ invoiceData, client }: InvoiceTemplateProps) {
    ...
    useEffect(() => {
        if(invoiceData.serviceDesc && !invoiceData.serviceDesc.startsWith('Being Costs of')) {
             setServiceDescription(`Being Costs of ${invoiceData.serviceDesc.replace(/^Provision of\s*/i, '')}`);
        } else {
    ...
    return (
        <div style={{ marginLeft: '1cm' }}>
             <Card id="invoice-pdf-content" className="p-8 bg-white text-black print:shadow-none" style={{ fontFamily: 'sans-serif', width: '19cm' }}>
                <div className="flex flex-col" style={{ minHeight: '25cm' }}>
                    ...
                    <div className="flex justify-between items-end mb-1">
                        <div className="flex-1">
                            <div className="text-base">
                                ...
                                <p className="mb-1 ml-6">{receiverName || client?.companyName}</p>
                                {receiverPosition && <p className="mb-1 ml-6">{receiverPosition}</p>}
                                {client && !receiverName && <p className="mb-1 ml-6">{client.companyName}</p>}
                                <p className="mb-1 ml-6">{client?.address1 || ''}</p>
                    ...
                    <div className="mt-auto pt-4" style={{ pageBreakInside: 'avoid' }}>
                        ...
                        <div className="flex justify-between items-end mt-4">
                            <div style={{fontSize: '12px'}}>
                                <p className="font-bold">Please remit your payment to the below Bank details:</p>
                                <div className="grid grid-cols-[max-content_auto] gap-x-2 gap-y-0">
