...
export function ProformaInvoiceTemplate({ invoiceData, client }: ProformaInvoiceTemplateProps) {
    ...
    return (
        <>
            <Card id="proforma-invoice-pdf-content" className="p-8 bg-white text-black print:shadow-none" style={{ fontFamily: 'sans-serif', position: 'relative', paddingBottom: '80px' }}>
              {/* HEADER SECTION */}
              ...
              <table className="w-full border-collapse border border-gray-800 mb-3">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td className="border border-black p-1 text-right font-semibold">{totalForDays.toLocaleString()}</td>
                    </tr>
                    <tr>
                        <td colSpan={3} className="border-r border-black"></td>
                        <td className="border-y border-black p-1 text-right">No of days</td>
                        <td className="border border-black p-1 text-right">{numberOfDays || '{No. of days}'}</td>
                    </tr>
                    <tr>
                        <td colSpan={3} className="border-r border-black"></td>
                        <td className="border-y border-black p-1 text-right">Add Service Charge (TSHS)</td>
                        <td className="border border-black p-1 text-right">{serviceCharge > 0 ? serviceCharge.toLocaleString() : '0.00'}</td>
                    </tr>
                    <tr>
                        <td colSpan={3} className="border-r border-black"></td>
                        <td className="border-y border-black p-1 text-right">Add Transportation Costs (TSHS)</td>
                        <td className="border border-black p-1 text-right">{transportCosts > 0 ? transportCosts.toLocaleString() : '0.00'}</td>
                    </tr>
                    <tr>
                        <td colSpan={3} className="border-r border-black"></td>
                        <td className="border-y border-black p-1 text-right">Add VAT 18% (TSHS)</td>
                        <td className="border border-black p-1 text-right">{vat ? vat.toLocaleString() : 'Inclusive'}</td>
                    </tr>
                    <tr>
                        <td colSpan={3} className="border-r border-black"></td>
                        <td className="border-y border-black p-1 text-right font-bold bg-secondary/40">GRAND TOTAL (TSHS)</td>
                        <td className="border border-black p-1 text-right font-bold bg-secondary/40 text-accent">{grandTotal ? grandTotal.toLocaleString() : '{GrandTotal}'}</td>
                    </tr>
                </tbody>
            </table>
