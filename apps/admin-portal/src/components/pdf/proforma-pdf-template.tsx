...
import { COMPANY_NAME, COMPANY_TIN, COMPANY_VRN, PROFORMA_TERMS, convertToWords, formatCurrency } from './pdf-shared';
...
export interface PdfItem {
  ...
  total: number;
  date?: string | null;
  particularDescription?: string | null;
  orderId?: string | null;
}
...
// Mirrors apps/catering-system/src/components/proforma-invoices/proforma-invoice-template.tsx
// layout and math exactly (including its quirk of showing "Inclusive"
// rather than a backed-out VAT figure for inclusive-type documents) — this
// is the actual printed document, not the corrected Tax Breakdown panel
// shown elsewhere in the portal.
export function ProformaPdfTemplate({ data, settings }: { data: ProformaPdfData; settings: AppSettings }) {
  const {
    id, invoiceDate, receiverName, receiverPosition, lpoNumber, serviceDesc, serviceCharge, transportCosts,
    multiplyByDays, numberOfDays, vatType, items,
  } = data;
    ...
    <div style={{ marginLeft: '1cm', marginRight: '0.8cm' }}>
      ...
      >
        ...
        <div id="proforma-main-content">
          <div className="text-right relative z-10" style={{ marginTop: '-4px' }}>
            <h2 className="font-bold text-4xl" style={{ color: '#800020' }}>
              PROFORMA INVOICE
            </h2>
            <div className="mt-1 text-base space-y-0">
              ...
              <p>
                <strong>Pro-Forma Invoice No.:</strong> {id || '{Invoice No.}'}
              </p>
            </div>
          </div>
          <div className="flex justify-between items-end mb-1">
            <div className="flex-1">
              <div className="text-base">
                ...
                {lpoNumber && (
                  <p className="mb-2 ml-6 pt-2 font-bold text-lg">LPO No.: {lpoNumber}</p>
                )}
              </div>
            </div>
            <div style={{ width: 220, position: 'relative', zIndex: 10 }}>
              <div className="border border-gray-800 flex flex-col items-center justify-center text-sm p-2 bg-white shadow-sm text-center">
                <div>
                  <strong>TIN: {COMPANY_TIN}</strong>
                </div>
                <div>
                  <strong>VRN: {COMPANY_VRN}</strong>
                </div>
              </div>
            </div>
          </div>
          ...
          <div data-pdf-no-break="true" className="flex gap-4 mt-6 mb-4 items-stretch">
            <div className="flex-1 border border-gray-800 p-3" style={{ fontSize: '14px' }}>
              ...
              <div style={{ fontSize: '14px', margin: 0 }}>
                {PROFORMA_TERMS.map((text, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', marginBottom: i < PROFORMA_TERMS.length - 1 ? '4px' : 0 }}>
        ...
        <div id="proforma-footer" className="pb-6">
          {settings.footerUrl && (
            <div className="mt-4">
              {/* eslint-disable-next-line @next/next/no-img-element */}
