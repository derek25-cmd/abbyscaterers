...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!isManager(session.role)) {
    return NextResponse.json({ error: 'Only managers or admins can reject commissions' }, { status: 403 });
  }
  ...
  const { data, error } = await client
    .from('marketer_commissions')
    .update({ status: 'REJECTED', reviewed_by: session.marketerId, reviewed_at: new Date().toISOString(), notes: notes ?? null })
    .eq('id', params.id)
    .select()
    .single();
