 ...
 * to the `anon` role there. Any table whose RLS policies require
 * `authenticated` needs a client scoped to this one request, forwarding the
 * caller's bearer token so `auth.uid()` resolves correctly.
 */
