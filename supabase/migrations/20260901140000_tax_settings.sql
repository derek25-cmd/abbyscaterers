...
INSERT INTO public.tax_rates (tax_type, rate) VALUES
  ('vat', 18.00),
  ('wht', 5.00),
...
CREATE POLICY "client_tax_settings_read" ON public.client_tax_settings
  FOR SELECT TO authenticated USING (public.is_active_portal_user() OR public.is_active_staff());
...
CREATE POLICY "client_tax_settings_write" ON public.client_tax_settings
  FOR ALL TO authenticated
  USING (public.has_portal_role(ARRAY['super_admin', 'finance']))
  WITH CHECK (public.has_portal_role(ARRAY['super_admin', 'finance']));
